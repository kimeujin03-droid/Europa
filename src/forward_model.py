...
def generate_spectrum(z: str, rng: np.random.Generator, cfg: ExperimentConfig, lib: dict[str, np.ndarray]) -> np.ndarray:
    # Draw mixture coefficients by hidden source class.
    if z == "ocean_organic":
        weights = {
            "ice": rng.uniform(0.35, 0.65),
            "ocean_salt": rng.uniform(0.18, 0.40),
            "simple_organic": rng.uniform(0.025, 0.095),
        }
    elif z == "ocean_nonorganic":
        weights = {
            "ice": rng.uniform(0.45, 0.72),
            "ocean_salt": rng.uniform(0.20, 0.45),
            "simple_organic": rng.uniform(0.0, 0.015),
        }
    elif z == "radiation_mimic":
        weights = {
            "ice": rng.uniform(0.35, 0.65),
            "ocean_salt": rng.uniform(0.10, 0.32),
            "simple_organic": rng.uniform(0.015, 0.060),
        ...
        weights = {
            ...
            "simple_organic": rng.uniform(0.00, 0.04),
        }
    else:
...
def generate_dataset(n: int, rho_geo: float, rho_rad: float, cfg: ExperimentConfig | None = None) -> pd.DataFrame:
    ...
    for i in range(n):
        ...
        spectrum = generate_spectrum(z, rng, cfg, lib)
